  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <div className="container py-8">
        ...
        {isEditing && <div className="rounded-xl border p-6 mb-8 space-y-4"><h2 className="text-xl font-bold">تعديل الملف الشخصي</h2><div><label className="text-sm font-semibold">الاسم</label><Input value={name} onChange={(e) => setName(e.target.value)} disabled={role === "author"} /></div><div><label className="text-sm font-semibold">البريد الإلكتروني</label><Input value={email} onChange={(e) => setEmail(e.target.value)} type="email" disabled={role === "author"} /></div><div><label className="text-sm font-semibold">الموقع الإلكتروني</label><Input value={website} onChange={(e) => setWebsite(e.target.value)} placeholder="https://example.com" /></div><div><label className="text-sm font-semibold">نبذة</label><Textarea value={bio} onChange={(e) => setBio(e.target.value)} maxLength={10000} /></div><Button disabled={isSaving || !specializedRole} onClick={saveProfile}>{isSaving ? "جاري الحفظ..." : "حفظ التغييرات"}</Button>{!specializedRole && <p className="text-sm text-muted-foreground">الملف العام للمستخدم العادي يحتاج endpoint مستقل لتعديل الاسم والبريد بأمان.</p>}</div>}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8"><Stat icon={<BookOpen />} label="إعلانات" value={listingsQuery.data?.length ?? 0} /><Stat icon={<Heart />} label="مفضلة" value={favoritesQuery.data?.length ?? 0} /><Stat icon={<Users />} label="متابعة" value={followingQuery.data?.length ?? 0} /><Stat icon={<Users />} label="متابعون" value={followersQuery.data?.length ?? 0} /></div>
        <Tabs defaultValue="listings"><TabsList className="grid w-full grid-cols-4"><TabsTrigger value="listings"><BookOpen className="h-4 w-4 ml-2" />إعلاناتي</TabsTrigger><TabsTrigger value="favorites"><Heart className="h-4 w-4 ml-2" />المفضلة</TabsTrigger><TabsTrigger value="following"><Users className="h-4 w-4 ml-2" />المتابعة</TabsTrigger><TabsTrigger value="messages"><MessageCircle className="h-4 w-4 ml-2" />الرسائل</TabsTrigger></TabsList><TabsContent value="listings" className="py-6"><div className="grid grid-cols-2 md:grid-cols-4 gap-4">{listingsQuery.data?.map((listing: any) => <div key={listing.listings?.id ?? listing.id} className="rounded-xl border p-4"><div className="font-semibold line-clamp-2">{listing.books?.title ?? listing.title ?? "إعلان"}</div><div className="text-sm text-muted-foreground mt-2">{listing.listings?.price ?? listing.price} {listing.listings?.currency ?? listing.currency}</div></div>)}</div>{!listingsQuery.data?.length && <p className="text-muted-foreground">لا توجد إعلانات.</p>}</TabsContent><TabsContent value="favorites" className="py-6"><p className="text-muted-foreground">عدد العناصر المحفوظة: {favoritesQuery.data?.length ?? 0}</p></TabsContent><TabsContent value="following" className="py-6"><p className="text-muted-foreground">عدد الحسابات التي تتابعها: {followingQuery.data?.length ?? 0}</p></TabsContent><TabsContent value="messages" className="py-6"><p className="text-muted-foreground">استخدم قسم الرسائل لإدارة محادثاتك.</p></TabsContent></Tabs>
      </div>
    </main>
  );
}

function Stat({ icon, label, value }: { icon: ReactNode; label: string; value: number }) { return <div className="rounded-xl border p-4 text-center"><div className="flex justify-center text-accent mb-2">{icon}</div><div className="text-2xl font-bold">{value}</div><div className="text-sm text-muted-foreground">{label}</div></div>; }
